...
Tabs.DupeTab:Button({
    Title = "Refresh Items",
    Callback = function()
        dupeDropdown:Refresh(UpdateItemList())
    end
})

-- Кнопка дюпа (визуальный)
Tabs.DupeTab:Button({
    Title = "Dupe Item",
    Callback = function()
        ...
        if not original then
            WindUI:Notify({
                Title = "Ошибка",
                Content = "Предмет не найден в инвентаре!",
                Icon = "x-circle",
                Duration = 3
            })
            return
        ...
        for i = 1, dupeAmount do
            ...
            task.wait(0.1) -- Чтобы избежать лагов
        end

        WindUI:Notify({
            Title = "Успех",
            Content = string.format("Создано %d копий %s", dupeAmount, selectedItem),
            Icon = "check-circle",
            Duration = 3
        })
    end
