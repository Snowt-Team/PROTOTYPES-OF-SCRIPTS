        ...
        if elements.Billboard then
            for _, child in ipairs(elements.Billboard:GetChildren()) do
                if child:IsA("TextLabel") then
                    if string.find(child.Text, player.Name) then
...
local function createHitboxVisuals(character)
    if not character then return end
    
    hitboxVisuals[character] = {}
    ...
    local originalSize = rootPart:GetAttribute("OriginalSize") or rootPart.Size
    rootPart.Size = originalSize * hitboxSettings.SizeMultiplier
    
    local highlight = Instance.new("Highlight")
    highlight.Name = "HitboxVisual"
    highlight.Adornee = character
    highlight.FillColor = hitboxSettings.Color
    highlight.FillTransparency = hitboxSettings.Transparency
    highlight.OutlineColor = hitboxSettings.Color
    highlight.OutlineTransparency = 0
    highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
    highlight.Parent = CoreGui
    
    hitboxVisuals[character].Highlight = highlight
end

local function removeHitboxVisuals(character)
    if not character or not hitboxVisuals[character] then return end
    
    local rootPart = character:FindFirstChild("HumanoidRootPart")
    if rootPart then
        local originalSize = rootPart:GetAttribute("OriginalSize")
        if originalSize then
            rootPart.Size = originalSize
        end
    end
    
    if hitboxVisuals[character].Highlight then
        hitboxVisuals[character].Highlight:Destroy()
    end
    
    hitboxVisuals[character] = nil
end

local function toggleCharacterHitboxes(character, enable)
    if not character then return end
    
    if enable then
        if hitboxSettings.ShowHitboxes then
            createHitboxVisuals(character)
        end
    else
        removeHitboxVisuals(character)
    end
end
...
local function onCharacterAdded(character)
    ...
    humanoid.Died:Connect(function()
        if hitboxVisuals[character] then
            removeHitboxVisuals(character)
        end
    end)
    
    toggleCharacterHitboxes(character, true)
end
...
local function initializeHitboxes()
    for _, visuals in pairs(hitboxVisuals) do
    ...
    if hitboxSettings.Enabled then
        ...
        table.insert(hitboxConnections, Players.PlayerAdded:Connect(onPlayerAdded))
        
        if LocalPlayer.Character then
            onCharacterAdded(LocalPlayer.Character)
        end
        table.insert(hitboxConnections, LocalPlayer.CharacterAdded:Connect(onCharacterAdded))
    end
end
...
Tabs.HitboxTab:Toggle({
    ...
    Callback = function(state)
        ...
        if hitboxSettings.Enabled then
            for character, _ in pairs(hitboxVisuals) do
                if state then
                    createHitboxVisuals(character)
                else
                    removeHitboxVisuals(character)
...
Tabs.HitboxTab:Slider({
    Title = "Hitbox Size",
    Value = {Min = 1, Max = 250, Default = 1, Step = 0.1},
    Callback = function(value)
        hitboxSettings.SizeMultiplier = value
        if hitboxSettings.Enabled and hitboxSettings.ShowHitboxes then
            for character, visuals in pairs(hitboxVisuals) do
                local rootPart = character:FindFirstChild("HumanoidRootPart")
                if rootPart then
                    local originalSize = rootPart:GetAttribute("OriginalSize") or rootPart.Size
                    rootPart.Size = originalSize * value
                end
            end
        end
    end
        ...
        if hitboxSettings.Enabled and hitboxSettings.ShowHitboxes then
            for _, visuals in pairs(hitboxVisuals) do
                if visuals.Highlight then
                    ...
                    visuals.Highlight.FillTransparency = transparency
...
Tabs.HitboxTab:Slider({
    ...
    Callback = function(value)
        ...
        if hitboxSettings.Enabled and hitboxSettings.ShowHitboxes then
            for _, visuals in pairs(hitboxVisuals) do
                if visuals.Highlight then
                    visuals.Highlight.FillTransparency = value
                end
            end
        end
    end
