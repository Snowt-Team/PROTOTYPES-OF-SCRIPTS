...
Tabs.CharacterTab:Section({Title = gradient("JumpPower [#001aff, #020524]", Color3.fromHex("#001aff"), Color3.fromHex("#020524"))})
...
local function CollectFruit()
    ...
    for _, object in pairs(workspace:GetDescendants()) do
        if object:IsA("ProximityPrompt") and object.MaxActivationDistance <= 15 then
            local promptPos = object.Parent:GetPivot().Position
            if (playerPos - promptPos).Magnitude <= 15 then
...
local function StartAutoWalk()
    if AutoWalkEnabled and not IsSelling then
        ...
        local DoRandom = AutoWalkAllowRandom
        if DoRandom then
