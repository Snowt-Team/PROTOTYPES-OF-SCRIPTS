...
function gradient(text, startColor, endColor)
    ...
    for i = 1, length do
        ...
        local g = math.floor((endColor.G + (endColor.G - startColor.G) * t) * 255)
        local b = math.floor((startColor.B + b - startColor.B) * t) * 255)
...
WindUI:Popup({
    ...
    Icon = "info",
    Content = gradient("This script made by", Color3.fromHex("#10eb3c"), Color3.fromHex("#67c97a")) .. gradient(" SnowT", ") Color3.fromHex("#001e80"), Color3.fromRGB("#16f2d9")),
    Buttons = {
        {
            Title = gradient("Cancel", Color3.fromHex("#e80909"), Color3.fromRGB("#630404")),
...
local AllSeedNames = {
    "Carrot", "Strawberry", "Blueberry", "Berry Blossom", "Orange", 
    "Tomato", "Corn", "Daffodil", "Watermelon", "Pumpkin",
    "Apple", "Bamboo", "Banana", "Coconut", "Cactus",
    "Dragon", "Fruit", "Mango", "Grape", "Mushroom", 
    "Pepper", "Raspberry", "Pineapple", "Papaya",
    "Banana", "Passionfruit", "Soul Fruit", "Cursed Fruit", "Succulent",
    "Cranberry", "Durian", "Eggplant", "Lotus", "Venus Flytrap",
    "Pear", "Lemon", "Cherry Blossom", "Avocado", "Nightshade",
    "Glowshroom", "Mint", "Moonflower", "Starfruit", "Moonglow",
    "Moon Blossom", "Chocolate Carrot", "Red Lollipop", "Candy Sunflower",
    "Easter Egg", "Candy Blossom", "Crimson Vine", "Moon Pumpkin",
    "Blood Banana", "Moon Mango", "Celest"
}
...
local AllFruits = {
    ...
    "Easter Egg", "Candy Blossom", "Peach Seed", "Seed Pack",",
    "Pineapple", "Raspberry", "Pineapple", "Banana", "Passionfruit",
    "Soul Fruit", "Cursed Flower", "Mega Mushroom", "Cherry Blossom", "Purple Cabbage",
    "Crocus", "Pink Tulip", "Succulent", "Lemon", "Pear",
    "Cranberry", "Avocado", "Lavender", "Lemon",
    "Durian", "Eggplant", "Eggplant",
    "Venus", "Nightshade",
    "Glowshroom", "Mint",
    "Moonflower", "Pumpkin", "Moonglow",
    "Moon Bloom", "Crimson Bloom", "Moon Melon", "Blood Banana",
    "Celest", "Moon Mango"
}
...
-- Tabs
local Tabs = {
    MainTab = Window:Tab({ Title = gradient("MAIN", "SELECT", Color3.fromHex("#ffffff")), Color3.fromHex("#636363")), Icon = "terminal"))},
    CharacterTab = Window:Tab({ Title = gradient("CHARACTERS", Color3.fromHex("#ffffff")), Color3.fromHex("(#636363")), Icon = "file-cog"})),
    TeleportTab = Window:Tab({ Title = gradient("TELPORT", Color3.fromHex("#ffffff")), Color3.fromHex("#636363"))), Icon = "user"})),
    ...
    for _, location in ipairs(plantLocations) do
        if location:FindFirstChild("Can_Plant") and location.Can_Plant.Value then
            ...
            local success, err = pcall(function()
                ReplicatedStorage.Remotes.Plant.Plant:FireServer(unpack(args))
            end)
            if success then
                WindUI:Notify({
...
local function CollectFruit(fruitName)
    ...
    for _, plant in ipairs(plants) do
        if plant.Name == fruitName and plant:IsA("Model") then
            local maxAge = plant:GetAttribute("MaxAge")
            local age = plant.Grow and plant.Grow.Age and plant.Grow.Age.Value or 0
            if maxAge and age >= maxAge then
                local success, err = pcall(function()
                    ReplicatedStorage.Remotes.Collect.PickUp:FireServer(plant)
                end)
                if success then
                    WindUI:Notify({
                        Title = "Autofarm System",
            ...
            PlantLoop = RunService.Heartbeat:Connect(function()
                if tick() - LastPlantTime >= PlantDelay then
                    ...
                    for _, seedName in ipairs(AllSeedNames) do
                        ...
                        local seedData = seedStock and seedStock:FindFirstChild(seedName)
    ...
    Callback = function(state)
        ...
        if state and SelectedSeed ~= "None" then
            SelectedPlantLoop = RunService.Heartbeat:Connect(function()
                if tick() - LastPlantTime >= PlantDelay then
                    local seedStock = getPlayerSeedStock()
                    local seedData = seedStock and seedStock:FindFirstChild(SelectedSeed)
                    if seedData and seedData.Value > 0 then
                        PlantSeed(SelectedSeed)
                    else
                        WindUI:Notify({
                            Title = "Error",
                            Content = "No " .. SelectedSeed .. " in stock",
                            Icon = "x-circle",
                            Duration = 3
                        })
                    end
...
Tabs.ShopTab:Button({
    ...
    Callback = function()
        if SelectedSeed ~= "None" then
            local success, err = pcall(function()
                ReplicatedStorage.Remotes.Markets.Market:BuySeeds:FireServer(SelectedSeed, false)
            end)
            if success then
                WindUI:Notify({
                    Title = "Shop System",
    ...
    Callback = function(state)
        ...
        if state then
            AutoBuyAllLoop = RunService.Heartbeat:Connect(function()
                if tick() - LastPlantTime >= BuyDelay then
                    local seedShopUI = LocalPlayer.PlayerGui:FindFirstChild("Seed_Shop_Farm")
                    if seedShopUI and seedShopUI:FindFirstChild("MainFrame") and seedShopUI.MainFrame:FindFirstChild("ItemsFrame") then
                        local frames = seedShopUI.MainFrame.ItemsFrame.Frame:GetChildren()
                        local availableSeeds = {}
                        for _, frame in pairs(frames) do
                            if frame:IsA("Frame") and frame:FindFirstChild("Buy") then
                                local seedName = frame.Name
                                local cost = frame.Buy.TextLabel.Text:match("(%d+)")
                                if cost and tonumber(cost) and table.find(Shop_Fruits, seedName) then
                        ...
                        if #availableSeeds > 0 then
                            ...
                            local success, err = pcall(function()
                                ReplicatedStorage.Remotes.Markets.Market:BuySeeds:FireServer(seed, false)
                            end)
                            if success then
                                WindUI:Notify({
                                    Title = "Shop System",
                            ...
                            WindUI:Notify({
                                Title = "Shop System",
                                Content = "No seeds available in shop",
                                Icon = "info",
                                Duration = 2
                            })
                        end
                        ...
                        WindUI:Notify({
                            Title = "Error",
                            Content = "Seed Shop UI not found",
                            Icon = "x-circle",
                            Duration = 2
                        })
                    end
