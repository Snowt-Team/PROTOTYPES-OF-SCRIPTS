...
-- Sheriff Tab (Updated to fix shooting issues)
...
local function CreateShotButton()
    ...
    local function animatePress()
        ...
        local pressTween = tweenService:Create(
            ...
            { Size = UDim2.new(0.9, 0, 0.9, 0), BackgroundTransparency = 0.7, TextColor3 = Color3.new(0.9, 0.9, 0.9) }
        )
        local strokeTween = tweenService:Create(
            stroke,
            TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
            { Color = Color3.fromRGB(0, 100, 255), Thickness = 1.8 }
        )
        local releaseTween = tweenService:Create(
            shotButton,
            TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
            { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 0.8, TextColor3 = Color3.new(1, 1, 1) }
        )
        local strokeReleaseTween = tweenService:Create(
            stroke,
            TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut),
            { Color = Color3.fromRGB(0, 150, 255), Thickness = 2 }
        )
        pressTween:Play()
        strokeTween:Play()
    ...
    Notify("Sheriff System", "Shot button activated", "check-circle")
end
...
local function RemoveShotButton()
    ...
    Notify("Sheriff System", "Shot button deactivated", "x-circle")
end

local function EquipGun()
    local character = LocalPlayer.Character
    if not character then return false end
    if character:FindFirstChild("Gun") then return true end
    local gun = LocalPlayer.Backpack:FindFirstChild("Gun")
    if gun then
        gun.Parent = character
        task.wait(0.2) -- Allow time for replication
        return character:FindFirstChild("Gun") ~= nil
    end
    return false
end

local function ShootMurderer()
    if not SetDebounce("ShootMurderer", Config.DebounceTime) then return end
    ...
    if not murderer or not murderer.Character or not murderer.Character:FindFirstChild("Humanoid") or murderer.Character.Humanoid.Health <= 0 then
        Notify("Sheriff System", "Murderer not found or not alive!", "x-circle")
        return
    end

    local targetRoot = murderer.Character:FindFirstChild("HumanoidRootPart")
    if not targetRoot then
        Notify("Sheriff System", "Murderer's HumanoidRootPart not found!", "x-circle")
        return
    end

    if shotType == "Default" and not EquipGun() then
        Notify("Sheriff System", "No gun found or failed to equip!", "x-circle")
        return
    end
    ...
    if not localRoot then
        Notify("Sheriff System", "Player's HumanoidRootPart not found!", "x-circle")
        return
    end

    if shotType == "Teleport" then
        localRoot.CFrame = targetRoot.CFrame * CFrame.new(0, 0, -4)
        task.wait(0.2) -- Allow time for teleport
        if not EquipGun() then
            Notify("Sheriff System", "No gun found or failed to equip after teleport!", "x-circle")
            return
        end
    end

    local gun = localCharacter:FindFirstChild("Gun")
    if not gun or not gun:FindFirstChild("KnifeLocal") or not gun.KnifeLocal:FindFirstChild("CreateBeam") or not gun.KnifeLocal.CreateBeam:IsA("RemoteFunction") then
        Notify("Sheriff System", "Gun not properly equipped or missing required components!", "x-circle")
        return
    end
    ...
    if success then
        ...
        Notify("Sheriff System", "Failed to fire shot: " .. tostring(err), "x-circle")
    end
end
...
Tabs.SheriffTab:Dropdown({
    ...
    Callback = function(selectedType)
        ...
        Notify("Sheriff System", "Shot Type set to: " .. selectedType, "check-circle")
    end
})
