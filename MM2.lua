...
-- AutoFarm (Optimized for Speed and Pathfinding)
local AutoFarm = {
    Enabled = false,
    Mode = "Teleport",
    TeleportDelay = 0,
    MoveSpeed = 50,
    WalkSpeed = 50, -- Increased default for faster Walk mode
    CoinCheckInterval = 0.1, -- Reduced default for faster retries
    UsePathfinding = true,
    DebugLogging = false,
    Connection = nil,
    CoinCache = {},
    LastPathCoin = nil, -- Cache last pathfinding target
    LastPath = nil, -- Cache last computed path
}
...
local function updateCoinCache()
        ...
        if map then
            ...
            if coinContainer then
                for _, obj in ipairs(coinContainer:GetChildren()) do
                    if obj.Name:match("Coin") and obj:IsA("BasePart") and obj.Parent then
                        table.insert(AutoFarm.CoinCache, obj)
                        if AutoFarm.DebugLogging then
                            print("[AutoFarm Debug] Found coin in CoinContainer:", obj.Name, "at", obj:GetFullName())
                        end
                    end
                end
            end
    ...
    -- Fallback: Search Workspace for stray coins
    for _, obj in ipairs(Workspace:GetChildren()) do
        if obj.Name:match("Coin") and obj:IsA("BasePart") and obj.Parent then
            table.insert(AutoFarm.CoinCache, obj)
            if AutoFarm.DebugLogging then
                print("[AutoFarm Debug] Found coin in Workspace:", obj.Name, "at", obj:GetFullName())
            end
        end
    end
    if AutoFarm.DebugLogging then
        print("[AutoFarm Debug] Total coins found:", #AutoFarm.CoinCache)
    end
end
...
local function findNearestCoin()
    ...
    for _, coin in ipairs(AutoFarm.CoinCache) do
        if coin and coin.Parent then
            local distance = (coin.Position - localPos).Magnitude
...
-- Helper: Collect Coin (Optimized)
local function collectCoin(coin)
    if not coin or not coin.Parent or not LocalPlayer.Character then return false end
    local humanoidRootPart = GetHumanoidRootPart(LocalPlayer)
    if not humanoidRootPart then return false end
    firetouchinterest(humanoidRootPart, coin, 0)
    firetouchinterest(humanoidRootPart, coin, 1)
    task.wait(0.05) -- Single wait for collection
    if not coin.Parent then
        if AutoFarm.DebugLogging then
            print("[AutoFarm Debug] Collected coin:", coin.Name)
        end
        return true
    end
    if AutoFarm.DebugLogging then
        print("[AutoFarm Debug] Failed to collect coin:", coin.Name)
    end
    return false
end

-- Helper: Teleport to Coin
local function teleportToCoin(coin)
    if not coin or not coin.Parent or not LocalPlayer.Character then return end
    local humanoidRootPart = GetHumanoidRootPart(LocalPlayer)
    if not humanoidRootPart then return end
    humanoidRootPart.CFrame = CFrame.new(coin.Position + Vector3.new(0, 3, 0))
    if AutoFarm.TeleportDelay > 0 then
        task.wait(AutoFarm.TeleportDelay)
    end
end

-- Helper: Smooth Move to Coin
local function smoothMoveToCoin(coin)
    ...
    local tween = TweenService:Create(humanoidRootPart, tweenInfo, {CFrame = CFrame.new(endPos)})
    tween:Play()
    tween.Completed:Wait()
end

-- Helper: Walk to Coin with Pathfinding (Optimized)
local function walkToCoin(coin)
    ...
    if not AutoFarm.UsePathfinding then
        humanoid:MoveTo(coin.Position + Vector3.new(0, 0, 3))
        local startTime = tick()
        while AutoFarm.Enabled and coin.Parent and tick() - startTime < 3 do
            if (humanoidRootPart.Position - coin.Position).Magnitude < 5 then break end
            task.wait(0.05)
        end
    else
        local path
        if AutoFarm.LastPathCoin == coin and AutoFarm.LastPath then
            path = AutoFarm.LastPath -- Reuse cached path
            if AutoFarm.DebugLogging then
                print("[AutoFarm Debug] Reusing cached path for coin:", coin.Name)
            end
        else
            path = PathfindingService:CreatePath({
                AgentRadius = 3, -- Increased for smoother paths
                AgentHeight = 5,
                AgentCanJump = true,
                WaypointSpacing = 2, -- Reduced for tighter paths
            })
            local success, _ = pcall(function()
                path:ComputeAsync(humanoidRootPart.Position, coin.Position + Vector3.new(0, 0, 3))
            end)
            if not success or path.Status ~= Enum.PathStatus.Success then
                if AutoFarm.DebugLogging then
                    print("[AutoFarm Debug] Pathfinding failed for coin:", coin.Name, "Status:", path.Status)
                end
                humanoid:MoveTo(coin.Position + Vector3.new(0, 0, 3))
                return
            end
            AutoFarm.LastPathCoin = coin
            AutoFarm.LastPath = path
            if AutoFarm.DebugLogging then
                print("[AutoFarm Debug] Computed new path for coin:", coin.Name)
            end
        end
        local waypoints = path:GetWaypoints()
        for _, waypoint in ipairs(waypoints) do
            if not AutoFarm.Enabled or not coin.Parent then break end
            humanoid:MoveTo(waypoint.Position)
            local startTime = tick()
            while AutoFarm.Enabled and tick() - startTime < 1 do -- Reduced timeout
                if (humanoidRootPart.Position - waypoint.Position).Magnitude < 2 then break end
                task.wait(0.05) -- Tighter check interval
            end
...
-- Helper: Farm Loop (Optimized)
local function farmLoop()
    updateCoinCache() -- Initial cache update
    while AutoFarm.Enabled do
        ...
        if coin and coin.Parent then
            ...
            if collectCoin(coin) then
                -- Remove collected coin from cache
                for i, cachedCoin in ipairs(AutoFarm.CoinCache) do
                    if cachedCoin == coin then
                        table.remove(AutoFarm.CoinCache, i)
                        break
                    end
                end
                if AutoFarm.LastPathCoin == coin then
                    AutoFarm.LastPathCoin = nil
                    AutoFarm.LastPath = nil
                end
            end
        else
...
-- Coin Monitoring (Dynamic Updates)
Workspace.DescendantAdded:Connect(function(child)
    if child.Name:match("Coin") and child:IsA("BasePart") then
        table.insert(AutoFarm.CoinCache, child)
        if AutoFarm.DebugLogging then
            print("[AutoFarm Debug] Coin added:", child.Name, "at", child:GetFullName())
        end
    elseif child.Name == "CoinContainer" then
        for _, coin in ipairs(child:GetChildren()) do
            if coin.Name:match("Coin") and coin:IsA("BasePart") then
                table.insert(AutoFarm.CoinCache, coin)
                if AutoFarm.DebugLogging then
                    print("[AutoFarm Debug] Coin added in CoinContainer:", coin.Name, "at", coin:GetFullName())
                end
            end
        end
        child.ChildAdded:Connect(function(coin)
            if coin.Name:match("Coin") and coin:IsA("BasePart") then
                table.insert(AutoFarm.CoinCache, coin)
                if AutoFarm.DebugLogging then
                    print("[AutoFarm Debug] Coin added dynamically:", coin.Name, "at", coin:GetFullName())
                end
            end
        end)
    end
...
Workspace.DescendantRemoving:Connect(function(child)
    if child.Name:match("Coin") and child:IsA("BasePart") then
        for i, coin in ipairs(AutoFarm.CoinCache) do
            if coin == child then
                table.remove(AutoFarm.CoinCache, i)
                if AutoFarm.LastPathCoin == child then
                    AutoFarm.LastPathCoin = nil
                    AutoFarm.LastPath = nil
                end
                if AutoFarm.DebugLogging then
                    print("[AutoFarm Debug] Coin removed:", child.Name)
                end
                break
            end
        end
...
Tabs.AutoFarm:Dropdown({
    Title = gradient("Movement Mode", Color3.fromHex("#FFD700"), Color3.fromHex("#FFA500")),
...
Tabs.AutoFarm:Toggle({
    Title = gradient("Use Pathfinding (Walk Mode)", Color3.fromHex("#FFD700"), Color3.fromHex("#FFA500")),
    Default = true,
    Callback = function(state)
        AutoFarm.UsePathfinding = state
        AutoFarm.LastPathCoin = nil
        AutoFarm.LastPath = nil
        Notify("AutoFarm", "Pathfinding " .. (state and "enabled" or "disabled") .. " for Walk mode", "check-circle", 2)
    end
})

Tabs.AutoFarm:Toggle({
    Title = gradient("Enable Debug Logging", Color3.fromHex("#FFD700"), Color3.fromHex("#FFA500")),
    Default = false,
    Callback = function(state)
        AutoFarm.DebugLogging = state
        Notify("AutoFarm", "Debug logging " .. (state and "enabled" or "disabled"), "check-circle", 2)
        if state then
            updateCoinCache()
        end
    end
...
Tabs.AutoFarm:Slider({
    Title = gradient("Teleport Delay (sec)", Color3.fromHex("#FFD700"), Color3.fromHex("#FFA500")),
    Value = {Min = 0, Max = 1, Default = 0, Step = 0.1},
    Callback = function(value)
        AutoFarm.TeleportDelay = value
        Notify("AutoFarm", "Teleport delay set to: " .. value .. "s", "check-circle", 2)
    end
})

Tabs.AutoFarm:Slider({
    Title = gradient("Smooth Move Speed", Color3.fromHex("#FFD700"), Color3.fromHex("#FFA500")),
    Value = {Min = 20, Max = 200, Default = 50},
    Callback = function(value)
        AutoFarm.MoveSpeed = value
        Notify("AutoFarm", "Smooth move speed set to: " .. value, "check-circle", 2)
    end
})
...
Tabs.AutoFarm:Slider({
    ...
    Callback = function(value)
        ...
        Notify("AutoFarm", "Walk speed set to: " .. value, "check-circle", 2)
    end
})

Tabs.AutoFarm:Slider({
    Title = gradient("Check Interval (sec)", Color3.fromHex("#FFD700"), Color3.fromHex("#FFA500")),
    Step = 0.1,
    Value = {Min = 0.1, Max = 2, Default = 0.1},
    Callback = function(value)
        AutoFarm.CoinCheckInterval = value
        Notify("AutoFarm", "Coin check interval set to: " .. value .. "s", "check-circle", 2)
    end
})
