...
-- Main Tab (Updated with Individual Paragraphs and Login Section)
...
local function SaveTelegramUsername(username)
    ...
    local filePath = folderPath .. "/telegram_username.json"
    local data = { Username = username }
    local success, err = pcall(function()
        writefile(filePath, HttpService:JSONEncode(data))
    end)
    if success then
        Notify("Login", "Telegram username saved successfully!", "check-circle")
    else
...
RunService.Heartbeat:Connect(function()
    ...
    fpsParagraph:Update({
        Desc = string.format("FPS: %.1f", PlayerInfo.FPS)
    })
    pingParagraph:Update({
        Desc = string.format("Ping: %.1f ms", PlayerInfo.Ping)
    })
    injectTimeParagraph:Update({
        Desc = string.format("Time: %s", FormatTimeSinceInject())
    })
    localTimeParagraph:Update({
        Desc = string.format("Time: %s", GetLocalTimeOfDay())
    })
end)
