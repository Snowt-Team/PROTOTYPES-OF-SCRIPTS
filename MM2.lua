...
local AutoFarm = {
    ...
    Mode = "Teleport",
    TeleportDelay = 0,
    MoveSpeed = 80, -- Increased for faster Smooth mode
    WalkSpeed = 80, -- Increased for faster Walk mode
    CoinCheckInterval = 0.3, -- Cache update interval
    UsePathfinding = true,
    DebugLogging = false,
    Connection = nil,
    CoinCache = {},
    ...
    for _, mapName in ipairs(mapPaths) do
        ...
        if map then
            ...
            if coinContainer then
                for i, coin in ipairs(coinContainer:GetChildren()) do
                    if coin.Name == "Coin_Server" and coin:IsA("BasePart") and coin.Parent then
                        AutoFarm.CoinCache[i] = coin
                        if AutoFarm.DebugLogging then
                            print("[AutoFarm Debug] Found Coin_Server at index", i, "in", coinContainer:GetFullName())
                        end
                    end
                end
            end
...
local function findNearestCoin()
    ...
    for _, coin in pairs(AutoFarm.CoinCache) do
...
-- Helper: Collect Coin (Move within 1 stud)
local function collectCoin(coin)
    ...
    if distance <= 1 then
        if AutoFarm.DebugLogging then
            print("[AutoFarm Debug] Collected Coin_Server:", coin.Name)
        end
        return true
    end
    return false
...
local function teleportToCoin(coin)
    ...
    humanoidRootPart.CFrame = CFrame.new(coin.Position + Vector3.new(0, 1, 0))
...
local function smoothMoveToCoin(coin)
    ...
    local endPos = coin.Position + Vector3.new(0, 1, 0)
...
local function walkToCoin(coin)
    ...
    if not AutoFarm.UsePathfinding then
        ...
        while AutoFarm.Enabled and coin.Parent and tick() - startTime < 1 do
            if (humanoidRootPart.Position - coin.Position).Magnitude < 1 then break end
            task.wait(0.03)
        end
    else
        local path
        if AutoFarm.LastPathCoin == coin and AutoFarm.LastPath then
            path = AutoFarm.LastPath
            if AutoFarm.DebugLogging then
                print("[AutoFarm Debug] Reusing cached path for Coin_Server:", coin.Name)
            end
        else
            path = PathfindingService:CreatePath({
                AgentRadius = 4, -- Increased for smoother paths
                AgentHeight = 5,
                AgentCanJump = true,
                WaypointSpacing = 1, -- Tighter paths
            })
            local success, _ = pcall(function()
                path:ComputeAsync(humanoidRootPart.Position, coin.Position)
            end)
            if not success or path.Status ~= Enum.PathStatus.Success then
                if AutoFarm.DebugLogging then
                    print("[AutoFarm Debug] Pathfinding failed for Coin_Server:", coin.Name, "Status:", path.Status)
                end
                humanoid:MoveTo(coin.Position)
                task.wait(0.3)
                return
            ...
            if AutoFarm.DebugLogging then
                print("[AutoFarm Debug] Computed new path for Coin_Server:", coin.Name)
            end
        end
        ...
        for _, waypoint in ipairs(waypoints) do
            ...
            local startTime = tick()
            while AutoFarm.Enabled and tick() - startTime < 0.5 do -- Tight timeout
                if (humanoidRootPart.Position - waypoint.Position).Magnitude < 1 then break end
                task.wait(0.03) -- Fast checks
            end
...
-- Helper: Farm Loop (No Delays, Fast Collection)
local function farmLoop()
    while AutoFarm.Enabled do
        updateCoinCache() -- Update every loop (0.3s when no coins)
        ...
        if coin and coin.Parent then
            ...
            if collectCoin(coin) then
                for i, cachedCoin in pairs(AutoFarm.CoinCache) do
                    if cachedCoin == coin then
                        AutoFarm.CoinCache[i] = nil
                        break
                    end
                end
            ...
            if AutoFarm.DebugLogging then
                print("[AutoFarm Debug] No coins found, waiting", AutoFarm.CoinCheckInterval, "seconds")
            end
...
Tabs.AutoFarm:Slider({
    ...
    Value = {Min = 16, Max = 100, Default = 80},
    Callback = function(value)
...
Tabs.AutoFarm:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            ...
            Notify("AutoFarm", "Started farming Coin_Server coins!", "check-circle", 2)
        else
