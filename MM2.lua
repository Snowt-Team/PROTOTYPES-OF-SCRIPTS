...
Tabs.AimbotTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            if not AimbotManager.Config.LockCamera then
                AimbotManager.Config.LockCamera = true
                Notify("Aimbot", "Camera lock enabled for Spectating Mode!", "Mode!", "check-circle", 2)
            end
            AimbotManager.Config.LockCamera = false -- Prevent lock conflict while spectating
        else
            CurrentCamera.CameraType = AimbotManager.Cache.OriginalCameraType
            CurrentCamera.CameraSubject = AimbotManager.Cache.OriginalCameraSubject
        end
        Notify("Aimbot", state and "Spectating Mode enabled!" or "Spectating Mode disabled", state and "disabled", state and "check-circle" or "x-circle", 2)
    end
})
