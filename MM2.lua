...
-- Murder Tab (Reworked with Kill All and Optimized for Speed)
local MurderTab = Tabs.MurderTab
local killAuraActive = false
local killAllActive = false
local killAuraRadius = 10
local attackDelay = 0.2 -- Reduced for faster attacks
...
-- Helper: Kill Aura Loop (Optimized)
            ...
            if player ~= LocalPlayer and IsAlive(player) then
                ...
                if targetRoot then
                    ...
                    if distance <= killAuraRadius then
                        attackPlayer(player, true)
                        attacked = true
                    end
                end
            end
        end
        if attacked and tick() - lastNotify >= 3 then
            Notify("Kill Aura", "Attacking nearby players", "check-circle", 2)
            lastNotify = tick()
        end
...
-- Helper: Kill All Loop (Optimized)
    ...
    while killAllActive do
        ...
        if #targets == 0 then
            if tick() - lastNotify >= 3 then
                Notify("Kill All", "No valid targets found", "x-circle", 2)
                lastNotify = tick()
            end
            task.wait(0.5)
            continue
        ...
        for _, target in ipairs(targets) do
            attackPlayer(target, true)
        end
        if tick() - lastNotify >= 3 then
            Notify("Kill All", "Attacking all players", "check-circle", 2)
            lastNotify = tick()
        end
        task.wait(attackDelay)
    end
    ...
    Callback = function(state)
        ...
        if state then
            if killAuraActive then
                ...
                Notify("Kill Aura", "Disabled Kill Aura to enable Kill All", "x-circle", 1)
            end
            Notify("Kill All", "Kill All enabled!", "check-circle", 2)
    ...
    Callback = function(state)
        ...
        if state then
            if killAllActive then
                killAllActive = false
                Notify("Kill All", "Disabled Kill All to enable Kill Aura", "x-circle", 1)
            end
...
Tabs.MurderTab:Slider({
    ...
    Value = {Min = 5, Max = 30, Default = 10, Step = 0.5},
    Callback = function(value)
        killAuraRadius = value
        Notify("Kill Aura", "Radius set to " .. value .. " studs", "check-circle", 1)
    end
})

Tabs.MurderTab:Slider({
    Title = gradient("Attack Delay", Color3.fromHex("#e80909"), Color3.fromHex("#630404")),
    Value = {Min = 0.1, Max = 1, Default = 0.2, Step = 0.05},
    Callback = function(value)
        attackDelay = value
        Notify("Kill Aura", "Attack delay set to " .. value .. "s", "check-circle", 1)
    end
})
