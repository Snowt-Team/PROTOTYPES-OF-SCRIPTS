...
-- Main Tab (Updated with Robust Map, Role, and Gun Holder Detection)
...
local function GetCurrentMap()
    -- Check direct map names in Workspace
    for _, mapName in ipairs(mapPaths) do
        if Workspace:FindFirstChild(mapName) then
            print("Map found in Workspace: " .. mapName)
            return mapName
        end
    end
    -- Check for maps in potential subfolders (e.g., Workspace.Maps)
    local mapsFolder = Workspace:FindFirstChild("Maps")
    if mapsFolder then
        for _, mapName in ipairs(mapPaths) do
            if mapsFolder:FindFirstChild(mapName) then
                print("Map found in Workspace.Maps: " .. mapName)
                return mapName
            end
        end
    end
    -- Fallback: Look for any Model with CoinContainer
    for _, child in ipairs(Workspace:GetChildren()) do
        if child:IsA("Model") and child:FindFirstChild("CoinContainer") then
            print("Map detected via CoinContainer: " .. child.Name)
            return child.Name
        end
    end
...
-- Helper: Get Gun Holder
local function GetGunHolder()
    for _, player in ipairs(Players:GetPlayers()) do
        -- Check Backpack
        if player.Backpack and player.Backpack:FindFirstChild("Gun") then
            print("Gun found in Backpack of: " .. player.Name)
            return player.Name
        end
        -- Check Character
        if player.Character and player.Character:FindFirstChild("Gun") then
            print("Gun found in Character of: " .. player.Name)
            return player.Name
        end
    end
    -- Check for dropped gun in Workspace
    local gun = Workspace:FindFirstChild("Gun")
    if gun then
        print("Gun found in Workspace (dropped)")
        return "Dropped"
    end
    print("No gun holder found")
    return "None"
end
...
local function UpdateRoles()
    ...
    if success and result then
        ...
        print("Roles updated via GetPlayerData")
    else
        Notify("Role Detection", "Failed to fetch role data, using tool-based fallback", "alert-circle", 5)
        ESPManager.Roles = {}
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= Players.LocalPlayer and player.Character then
                local tool = player.Character:FindFirstChildOfClass("Tool")
                local backpackTool = player.Backpack and player.Backpack:FindFirstChildOfClass("Tool")
                if (tool and tool.Name:lower():match("knife")) or (backpackTool and backpackTool.Name:lower():match("knife")) then
                    ESPManager.Roles[player.Name] = {Role = "Murderer"}
                    print("Murderer detected: " .. player.Name)
                elseif (tool and tool.Name:lower():match("gun")) or (backpackTool and backpackTool.Name:lower():match("gun")) then
                    ESPManager.Roles[player.Name] = {Role = "Sheriff"}
                    print("Sheriff detected: " .. player.Name)
                else
...
-- FPS Paragraph
local fpsParagraph = MainTab:Paragraph({
...
-- Ping Paragraph
local pingParagraph = MainTab:Paragraph({
...
local injectTimeParagraph = MainTab:Paragraph({
...
local localTimeParagraph = MainTab:Paragraph({
...
local currentPlayersParagraph = MainTab:Paragraph({
...
local maxPlayersParagraph = MainTab:Paragraph({
...
MainTab:Section({Title = gradient("Game Information", Color3.fromHex("#ffffff"), Color3.fromHex("#636363"))})

-- Current Map Paragraph
local currentMapParagraph = MainTab:Paragraph({
    Title = gradient("Current Map", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    Desc = "Map: Unknown",
    Color = "Grey",
    Image = "info",
    ImageSize = 30,
...
-- Murderer Paragraph
local murdererParagraph = MainTab:Paragraph({
    Title = gradient("Murderer", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    Desc = "Murderer: None",
    Color = "Grey",
    Image = "info",
    ImageSize = 30,
...
-- Sheriff Paragraph
local sheriffParagraph = MainTab:Paragraph({
    Title = gradient("Sheriff", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    Desc = "Sheriff: None",
    Color = "Grey",
    Image = "info",
    ImageSize = 30,
...
-- Gun Holder Paragraph
local gunHolderParagraph = MainTab:Paragraph({
    Title = gradient("Gun Holder", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    Desc = "Gun Holder: None",
    Color = "Grey",
    Image = "info",
    ImageSize = 30,
    Thumbnail = "",
...
-- Update Loop for Player and Game Info
local lastUpdate = 0
RunService.Heartbeat:Connect(function()
    ...
    -- Update Game Info
    GameInfo.CurrentMap = GetCurrentMap()
    UpdateRoles()
    GameInfo.Murderer = "None"
    GameInfo.Sheriff = "None"
    for playerName, data in pairs(ESPManager.Roles) do
        if data.Role == "Murderer" then
            GameInfo.Murderer = playerName
        elseif data.Role == "Sheriff" or data.Role == "Hero" then
            GameInfo.Sheriff = playerName
        end
    end
    GameInfo.GunHolder = GetGunHolder()

    currentMapParagraph:SetDesc(string.format("Map: %s", GameInfo.CurrentMap))
    murdererParagraph:SetDesc(string.format("Murderer: %s", GameInfo.Murderer))
    sheriffParagraph:SetDesc(string.format("Sheriff: %s", GameInfo.Sheriff))
    gunHolderParagraph:SetDesc(string.format("Gun Holder: %s", GameInfo.GunHolder))

    -- Debug output
    print(string.format("Map: %s, Murderer: %s, Sheriff: %s, Gun Holder: %s", 
        GameInfo.CurrentMap, GameInfo.Murderer, GameInfo.Sheriff, GameInfo.GunHolder))
end)

-- Login Section
MainTab:Section({Title = gradient("Login", Color3.fromHex("#ffffff"), Color3.fromHex("#636363"))})

local telegramUsername = ""
MainTab:Input({
    Title = gradient("Telegram Username", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    PlaceholderText = "Enter your Telegram username",
...
MainTab:Button({
    Title = gradient("Save Username", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    Callback = function()
        SaveTelegramUsername(telegramUsername)
    end
})

local usernameDropdown
usernameDropdown = MainTab:Dropdown({
    Title = gradient("Saved Usernames", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    Multi = false,
    AllowNone = true,
    Values = ListSavedUsernames(),
    Callback = function(selected)
        if selected then
            telegramUsername = selected
            Notify("Login", "Selected username: " .. selected, "check-circle")
        end
    end
})

MainTab:Button({
    Title = gradient("Refresh Dropdown", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    Callback = function()
        usernameDropdown:Refresh(ListSavedUsernames())
        Notify("Login", "Username list refreshed", "check-circle")
    end
})

MainTab:Button({
    Title = gradient("Verify Username", Color3.fromHex("#00eaff"), Color3.fromHex("#002a2e")),
    Callback = function()
        if telegramUsername ~= "" then
            VerifyUsername(telegramUsername)
        else
            Notify("Login", "No username selected or entered", "x-circle")
        end
    end
})
