...
-- AutoFarm (Reworked for Speed and Smart Pathfinding)
local AutoFarm = {
    Enabled = false,
    Mode = "Teleport",
    TeleportDelay = 0,
    ...
    UsePathfinding = true, -- New: Toggle for smart walking
    Connection = nil,
    CoinCache = {}, -- Cache for active coins
}
...
local function collectCoin(coin)
    ...
    for _ = 1, 3 do -- Retry up to 3 times
        ...
        task.wait(0.05) -- Minimal delay between attempts
        if not coin.Parent then return true end -- Coin collected
    end
...
local function teleportToCoin(coin)
    ...
    if AutoFarm.TeleportDelay > 0 then
        task.wait(AutoFarm.TeleportDelay) -- Apply delay only if configured
    end
end
...
local function walkToCoin(coin)
    ...
    if not AutoFarm.UsePathfinding then
        -- Simple MoveTo (faster but no obstacle avoidance)
        ...
        -- Pathfinding (smarter but slightly slower)
...
local function farmLoop()
    updateCoinCache() -- Initialize coin cache
    while AutoFarm.Enabled do
        ...
        if coin and coin.Parent then
            ...
            if collectCoin(coin) then
                updateCoinCache() -- Refresh cache after collection
            end
        else
            Notify("AutoFarm", "No coins found nearby!", "alert-circle", 2)
            updateCoinCache() -- Refresh cache to check for new coins
            task.wait(AutoFarm.CoinCheckInterval)
        end
        task.wait(0.05) -- Minimal loop delay to prevent freezing
    end
end
