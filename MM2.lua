...
-- AutoFarm (Reworked for Robust Coin Detection)
local AutoFarm = {
    Enabled = false,
    Mode = "Teleport",
    TeleportDelay = 0,
    ...
    UsePathfinding = true,
    DebugLogging = false, -- New: Toggle for debug prints
    Connection = nil,
    CoinCache = {},
}
    ...
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj.Name:match("Coin") and obj:IsA("BasePart") and obj.Parent then
            ...
            if AutoFarm.DebugLogging then
                print("[AutoFarm Debug] Found coin:", obj.Name, "at", obj:GetFullName())
            end
        end
    end
...
-- Helper: Walk to Coin with Pathfinding
local function walkToCoin(coin)
    ...
    if not AutoFarm.UsePathfinding then
        ...
        while AutoFarm.Enabled and coin.Parent and tick() - startTime < 5 do
        ...
        if success and path.Status == Enum.PathStatus.Success then
            ...
            for _, waypoint in ipairs(waypoints) do
                ...
                local startTime = tick()
                while AutoFarm.Enabled and tick() - startTime < 3 do
...
Tabs.AutoFarm:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            updateCoinCache() -- Trigger immediate cache update to log coins
        end
    end
})
