...
-- Murder Tab (Completely Reworked)
...
local function equipKnife()
    ...
    if not character then
        Notify("Knife", "Player character not found", "x-circle", 2)
        return false
    end
    ...
    Notify("Knife", "No knife found in backpack", "x-circle", 2)
    return false
end

-- Helper: Attack Player
local function attackPlayer(target)
    if not SetDebounce("AttackPlayer", Config.DebounceTime) then return false end
    if not target or not target.Character then
        Notify("Kill", "Invalid target", "x-circle", 2)
        return false
    end
    local humanoid = target.Character:FindFirstChild("Humanoid")
    if not humanoid or humanoid.Health <= 0 then
        Notify("Kill", target.Name .. " is already dead", "x-circle", 2)
        return false
    end
    ...
    Notify("Kill", "Failed to attack " .. target.Name, "x-circle", 2)
    return false
end

-- Helper: Get Sheriff or Hero
local function getSheriffOrHero()
    ...
    Notify("Kill Sheriff", "Failed to fetch roles or no Sheriff/Hero found", "x-circle", 2)
    return nil
end

-- Helper: Kill Aura Loop
local function killAuraLoop()
    while killAuraActive do
        local localRoot = GetHumanoidRootPart(LocalPlayer)
        if not localRoot or not equipKnife() then
            task.wait(0.5)
            continue
        end
...
Tabs.MurderTab:Button({
    ...
    Callback = function()
        ...
        if sheriff then
            attackPlayer(sheriff)
        else
            Notify("Kill Sheriff", "No Sheriff or Hero in the match", "x-circle", 2)
        end
    end
})
...
playerDropdown = Tabs.MurderTab:Dropdown({
    ...
    Callback = function(selected)
        ...
        if teleportTarget then
            Notify("Kill Player", "Selected " .. selected .. " as target", "check-circle", 2)
        end
    end
})
...
Tabs.MurderTab:Button({
    ...
    Callback = function()
        ...
        Notify("Kill Player", "Player list refreshed", "check-circle", 2)
    end
})

-- Kill Aura
Tabs.MurderTab:Section({Title = gradient("Kill Aura", Color3.fromHex("#e80909"), Color3.fromHex("#630404"))})

Tabs.MurderTab:Toggle({
    Title = gradient("Enable Kill Aura", Color3.fromHex("#e80909"), Color3.fromHex("#630404")),
    Default = false,
    Callback = function(state)
        killAuraActive = state
        if state then
            Notify("Kill Aura", "Kill Aura enabled!", "check-circle", 2)
            task.spawn(killAuraLoop)
        else
            Notify("Kill Aura", "Kill Aura disabled", "x-circle", 2)
        end
    end
})

Tabs.MurderTab:Slider({
    Title = gradient("Kill Aura Radius", Color3.fromHex("#e80909"), Color3.fromHex("#630404")),
    Value = {Min = 5, Max = 50, Default = 10, Step = 0.5},
    Callback = function(value)
        killAuraRadius = value
        Notify("Kill Aura", "Radius set to " .. value .. " studs", "check-circle", 2)
    end
})

Tabs.MurderTab:Slider({
    Title = gradient("Attack Delay", Color3.fromHex("#e80909"), Color3.fromHex("#630404")),
    Value = {Min = 0.1, Max = 2, Default = 0.5, Step = 0.1},
    Callback = function(value)
        attackDelay = value
        Notify("Kill Aura", "Attack delay set to " .. value .. " seconds", "check-circle", 2)
    end
})
...
Players.PlayerRemoving:Connect(function(player)
    ...
    if player == teleportTarget then
        teleportTarget = nil
        Notify("Kill Player", player.Name .. " left the game, target reset", "x-circle", 2)
    end
end)
