...
local function GrabAndShootMurderer()
    if not SetDebounce("GrabAndShoot", Config.DebounceTime) then return end
    if not (LocalPlayer.Character and (LocalPlayer.Character:FindFirstChild("Gun") or GrabGun())) then
        Notify("Gun System", "Failed to get or equip gun!", "x-circle")
        return
    end

    if not EquipGun() then
        Notify("Gun System", "Failed to equip gun!", "x-circle")
        return
    end

    local murderer = GetMurderer()
    if not murderer or not murderer.Character or not IsAlive(murderer) then
        Notify("Gun System", "Murderer not found or unavailable!", "x-circle")
        return
    end

    local targetRoot = GetHumanoidRootPart(murderer)
    local localRoot = GetHumanoidRootPart(LocalPlayer)
    if targetRoot and localRoot then
    ...
    if not gun then
        Notify("Gun System", "Gun not equipped!", "x-circle")
        return
    end

    local targetPart = GetHumanoidRootPart(murderer)
    if targetPart then
        local args = {1, targetPart.Position, "AH2"}
        if gun:FindFirstChild("KnifeLocal") and gun.KnifeLocal:FindFirstChild("CreateBeam") then
            ...
            Notify("Gun System", "Successfully shot the murderer!", "check-circle")
        end
    end
end
...
Tabs.InnocentTab:Toggle({
    ...
    Default = false,
    Callback = function(state)
        GunSystem.AutoShootEnabled = state
        Notify("Gun System", "Auto Shoot Murderer " .. (state and "enabled" or "disabled"), "check-circle")
    end
})
Tabs.InnocentTab:Button({
...
-- Sheriff Tab (unchanged for brevity)
local shotButton, shotButtonFrame = nil, nil
...
local function CreateShotButton()
    ...
    local function animatePress()
        local pressTween = tweenService:Create(shotButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
            Size = UDim2.new(0, buttonSize * 0.9, 0, buttonSize * 0.9),
            BackgroundTransparency = 0.7,
            TextColor3 = Color3.new(0.9, 0.9, 0.9)
        })
        local strokeTween = tweenService:Create(stroke, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
            Color = Color3.fromRGB(0, 100, 255),
            Thickness = 1.8
        })
        local releaseTween = tweenService:Create(shotButton, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
            Size = UDim2.new(0, buttonSize, 0, buttonSize),
            BackgroundTransparency = 0.8,
            TextColor3 = Color3.new(1, 1, 1)
        })
        local strokeReleaseTween = tweenService:Create(stroke, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {
    ...
    shotButton.Activated:Connect(function()
        ...
        local success, roles = pcall(function() return ReplicatedStorage:FindFirstChild("GetPlayerData", true):InvokeServer() end)
        if not success or not roles then return end
        local murderer
        for name, data in pairs(roles) do
            if data.Role == "Murderer" then murderer = Players:FindFirstChild(name) break end
        end
        if not murderer or not murderer.Character or not murderer.Character:FindFirstChild("Humanoid") or not murderer.Character:FindFirstChild("HumanoidRootPart") then return end
        local localCharacter = LocalPlayer.Character
        local localRoot = GetHumanoidRootPart(localCharacter)
        local gun = localCharacter:FindFirstChild("Gun") or LocalPlayer.Backpack:FindFirstChild("Gun")
        if shotType == "Default" and not gun then return end
        if gun and gun.Parent ~= localCharacter then gun.Parent = localCharacter end
        if shotType == "Teleport" then
            local targetRoot = GetHumanoidRootPart(murderer)
            if targetRoot and localRoot then localRoot.CFrame = targetRoot.CFrame * CFrame.new(0, 0, -4) end
        end
        if gun and gun:FindFirstChild("KnifeLocal") then
            local targetPart = GetHumanoidRootPart(murderer)
            if targetPart then gun.KnifeLocal.CreateBeam.RemoteFunction:InvokeServer(1, targetPart.Position) end
        end
    end)
    ...
    local function update(input)
        ...
        shotButtonFrame.Position = UDim2.new(0, math.clamp(startPos.X.Offset + delta.X, 0, guiSize.X - buttonSize), 0, math.clamp(startPos.Y.Offset + delta.Y, 0, guiSize.Y - buttonSize))
    end
...
local function RemoveShotButton()
    ...
    Notify("Shot Button", "Deactivated", "check-circle")
end
...
local function ShootMurderer()
    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("Humanoid") or LocalPlayer.Character.Humanoid.Health <= 0 then
        Notify("Sheriff System", "Player not alive!", "x-circle")
        return
    end
    local success, roles = pcall(function() return ReplicatedStorage:FindFirstChild("GetPlayerData", true):InvokeServer() end)
    if not success or not roles then
        Notify("Sheriff System", "Failed to fetch roles!", "x-circle")
        return
    end
    local murderer
    for name, data in pairs(roles) do
    ...
    if not murderer or not murderer.Character or not murderer.Character:FindFirstChild("Humanoid") or murderer.Character.Humanoid.Health <= 0 then
        Notify("Sheriff System", "Murderer not found!", "x-circle")
        return
    end
    local localCharacter = LocalPlayer.Character
    local localRoot = GetHumanoidRootPart(localCharacter)
    ...
    if shotType == "Default" and not gun then
        Notify("Sheriff System", "No gun equipped!", "x-circle")
        return
    end
    if gun and gun.Parent ~= localCharacter then gun.Parent = localCharacter end
    if shotType == "Teleport" then
    ...
    if gun and gun:FindFirstChild("KnifeLocal") then
        ...
        if targetPart then
            ...
            Notify("Sheriff System", "Shot fired at murderer!", "check-circle")
        end
    end
end
...
Tabs.SheriffTab:Dropdown({
    ...
    Callback = function(selectedType)
        ...
        Notify("Sheriff System", "Shot Type: " .. selectedType, "check-circle")
    end
})
...
Tabs.SheriffTab:Slider({
    ...
    Callback = function(value)
        ...
        Notify("Sheriff System", "Button size set to: " .. value, "check-circle")
    end
})
Tabs.SheriffTab:Toggle({
    ...
    Default = false,
    Callback = function(state)
        if state then CreateShotButton() else RemoveShotButton() end
    end
})
