...
-- ESP Manager (Optimized and Modular)
local ESPManager = {
    Config = {
...
local function CreateTracer(playerOrGun, color)
    ...
    line.Transparency = ESPManager.Config.Tracers.Transparency
    line.ZIndex = 10
    line.Parent = CoreGui
    return line
end
...
local function RemoveEsp(target)
    ...
    if ESPManager.Cache.Tracers[target] then
        ESPManager.Cache.Tracers[target]:Destroy()
            ...
            if ESPManager.Config.Tracers[espRole] then
                ...
                if ESPManager.Cache.Tracers[player] then
                    ...
                    if head then
                        ...
                        if onScreen then
                            local startPos = Vector3.new(CurrentCamera.ViewportSize.X / 2, CurrentCamera.ViewportSize.Y, 0)
                            local endPos = Vector3.new(screenPos.X, screenPos.Y, screenPos.Z)
                            ESPManager.Cache.Tracers[player].CFrame = CFrame.new(startPos, endPos)
                            ESPManager.Cache.Tracers[player].Length = (startPos - endPos).Magnitude / 10
                ...
                if ESPManager.Cache.Tracers[player] then
                    ESPManager.Cache.Tracers[player]:Destroy()
    ...
    if ESPManager.Cache.GunDrop then
        ...
        if ESPManager.Config.Tracers.GunDrop then
            ...
            if ESPManager.Cache.Tracers[ESPManager.Cache.GunDrop] then
                local screenPos, onScreen = CurrentCamera:WorldToScreenPoint(ESPManager.Cache.GunDrop.Position)
                if onScreen then
                    local startPos = Vector3.new(CurrentCamera.ViewportSize.X / 2, CurrentCamera.ViewportSize.Y, 0)
                    local endPos = Vector3.new(screenPos.X, screenPos.Y, screenPos.Z)
                    ESPManager.Cache.Tracers[ESPManager.Cache.GunDrop].CFrame = CFrame.new(startPos, endPos)
                    ESPManager.Cache.Tracers[ESPManager.Cache.GunDrop].Length = (startPos - endPos).Magnitude / 10
            ...
            if ESPManager.Cache.Tracers[ESPManager.Cache.GunDrop] then
                ESPManager.Cache.Tracers[ESPManager.Cache.GunDrop]:Destroy()
...
Tabs.EspTab:Slider({
    ...
    Callback = function(value)
        ...
        for _, tracer in pairs(ESPManager.Cache.Tracers) do
            if tracer then tracer.Transparency = value end
        end
    end
})
