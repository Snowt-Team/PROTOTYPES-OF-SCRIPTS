...
-- Main Tab (Updated with Fixed Map and Role Paragraphs)
...
local function GetCurrentMap()
    ...
    -- Fallback: Check all models in Workspace for map-like structure
...
local function UpdateRoles()
    ...
    if success and result then
        ...
        Notify("ESP Error", "Failed to fetch role data, using fallback", "alert-circle", 5)
        ESPManager.Roles = {}
        for _, player in ipairs(Players:GetPlayers()) do
...
Tabs.MainTab:Section({Title = gradient("Game Information", Color3.fromHex("#ffffff"), Color3.fromHex("#636363"))})

-- Current Map Paragraph
local currentMapParagraph = Tabs.MainTab:Paragraph({
...
-- Murderer Paragraph
local murdererParagraph = Tabs.MainTab:Paragraph({
...
-- Sheriff Paragraph
local sheriffParagraph = Tabs.MainTab:Paragraph({
...
local gunHolderParagraph = Tabs.MainTab:Paragraph({
...
RunService.Heartbeat:Connect(function()
    if tick() - lastUpdate < 0.2 then return end -- Throttle updates to prevent UI lag
    lastUpdate = tick()
...
-- Login Section
Tabs.MainTab:Section({Title = gradient("Login", Color3.fromHex("#ffffff"), Color3.fromHex("#636363"))})
...
usernameDropdown = Tabs.MainTab:Dropdown({
