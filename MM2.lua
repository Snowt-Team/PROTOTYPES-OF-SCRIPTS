...
-- Aimbot Manager (Reworked with Fixed FOV and Spectating Mode)
local AimbotManager = {
    Config = {
        Enabled = false,
        TargetRole = "None",
        ...
        Spectate = {
            Enabled = false,
            Offset = CFrame.new(0, 5, 10), -- Camera offset for spectating
        },
        FOV = {
            Enabled = true,
            Radius = 150,
    ...
    Cache = {
        FOVCircle = nil,
        Connection = nil,
        InputConnectionBegan = nil,
        InputConnectionEnded = nil,
        IsActive = false,
            ...
            if AimbotManager.Config.TargetRole == aimRole or AimbotManager.Config.TargetRole == "All" then
                ...
                if head then
                    ...
                    if onScreen then
                        ...
                        if screenDistance <= AimbotManager.Config.FOV.Radius then
...
local function CreateFOVCircle()
    if not AimbotManager.Config.FOV.Enabled then return end
    ...
    circle.Visible = AimbotManager.Config.Enabled
    circle.Filled = false
...
local function UpdateFOVCircle()
    if AimbotManager.Cache.FOVCircle then
        ...
        AimbotManager.Cache.FOVCircle.Visible = AimbotManager.Config.FOV.Enabled and AimbotManager.Config.Enabled
    elseif AimbotManager.Config.FOV.Enabled and AimbotManager.Config.Enabled then
        CreateFOVCircle()
    end
end
...
local function CleanupAimbot()
    AimbotManager.Config.Enabled = false
    AimbotManager.Config.Spectate.Enabled = false
    AimbotManager.Cache.IsActive = false
    if AimbotManager.Cache.Connection then
        AimbotManager.Cache.Connection:Disconnect()
        AimbotManager.Cache.Connection = nil
    end
    if AimbotManager.Cache.InputConnectionBegan then
        AimbotManager.Cache.InputConnectionBegan:Disconnect()
        AimbotManager.Cache.InputConnectionBegan = nil
    end
    if AimbotManager.Cache.InputConnectionEnded then
        AimbotManager.Cache.InputConnectionEnded:Disconnect()
        AimbotManager.Cache.InputConnectionEnded = nil
    end
    if AimbotManager.Cache.FOVCircle then
...
local function InitializeAimbot()
    ...
    -- Input Handling
    AimbotManager.Cache.InputConnectionBegan = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if not gameProcessed and input.UserInputType == AimbotManager.Config.Keybind then
            AimbotManager.Cache.IsActive = true
        end
    end)

    AimbotManager.Cache.InputConnectionEnded = UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == AimbotManager.Config.Keybind then
            AimbotManager.Cache.IsActive = false
        end
    end)
    ...
    AimbotManager.Cache.Connection = RunService.RenderStepped:Connect(function(deltaTime)
        ...
        if not AimbotManager.Config.Enabled then
            if AimbotManager.Config.Spectate.Enabled then
                CurrentCamera.CameraType = AimbotManager.Cache.OriginalCameraType
                CurrentCamera.CameraSubject = AimbotManager.Cache.OriginalCameraSubject
            end
            return
        ...
        elseif AimbotManager.Config.Spectate.Enabled then
            CurrentCamera.CameraType = AimbotManager.Cache.OriginalCameraType
            CurrentCamera.CameraSubject = AimbotManager.Cache.OriginalCameraSubject
        elseif AimbotManager.Cache.IsActive and target then
...
Tabs.AimbotTab:Toggle({
    ...
    Default = false,
    Callback = function(state)
        AimbotManager.Config.Enabled = state
        if not state then
            AimbotManager.Cache.IsActive = false
            AimbotManager.Config.Spectate.Enabled = false
            if AimbotManager.Cache.FOVCircle then
        ...
        Notify("Aimbot", state and "Aimbot enabled!" or "Aimbot disabled", state and "check-circle" or "x-circle", 2)
    end
})
...
Tabs.AimbotTab:Dropdown({
    Title = gradient("Aimbot Keybind", Color3.fromHex("#00448c"), Color3.fromHex("#0affd6")),
    Values = {"MouseButton1", "MouseButton2", "MouseButton3"},
    Value = "MouseButton2",
    Callback = function(selected)
        AimbotManager.Config.Keybind = Enum.UserInputType[selected]
        Notify("Aimbot", "Keybind set to: " .. selected, "check-circle", 2)
    end
})
...
Tabs.AimbotTab:Toggle({
    Title = gradient("Enable FOV Circle", Color3.fromHex("#00448c"), Color3.fromHex("#0affd6")),
    Default = true,
    Callback = function(state)
        AimbotManager.Config.FOV.Enabled = state
        if AimbotManager.Cache.FOVCircle then
            AimbotManager.Cache.FOVCircle.Visible = state and AimbotManager.Config.Enabled
        end
        Notify("Aimbot", state and "FOV Circle enabled!" or "FOV Circle disabled", state and "check-circle" or "x-circle", 2)
    end
})
Tabs.AimbotTab:Slider({
...
Tabs.AimbotTab:Section({Title = gradient("Spectate Settings", Color3.fromHex("#00448c"), Color3.fromHex("#0affd6"))})
Tabs.AimbotTab:Toggle({
    Title = gradient("Spectating Mode", Color3.fromHex("#00448c"), Color3.fromHex("#0affd6")),
    Default = false,
    Callback = function(state)
        AimbotManager.Config.Spectate.Enabled = state
        if state then
            if not AimbotManager.Config.Enabled then
                AimbotManager.Config.Enabled = true
                Notify("Aimbot", "Aimbot enabled for Spectating Mode!", "check-circle", 2)
            end
            AimbotManager.Cache.IsActive = false -- Disable aimbot lock while spectating
        else
            CurrentCamera.CameraType = AimbotManager.Cache.OriginalCameraType
            CurrentCamera.CameraSubject = AimbotManager.Cache.OriginalCameraSubject
        end
        Notify("Aimbot", state and "Spectating Mode enabled!" or "Spectating Mode disabled", state and "check-circle" or "x-circle", 2)
    end
})
