...
local AimbotManager = {
    Config = {
        ...
        Keybind = Enum.UserInputType.MouseButton2, -- Right mouse button by default
    },
    Cache = {
        FOVCircle = nil,
        Connection = nil,
...
local function UpdateFOVCircle()
    if AimbotManager.Cache.FOVCircle then
        ...
        AimbotManager.Cache.FOVCircle.Position = UserInputService:GetMouseLocation()
...
local function CleanupAimbot()
    ...
    if AimbotManager.Cache.InputConnection then
        AimbotManager.Cache.InputConnection:Disconnect()
        AimbotManager.Cache.InputConnection = nil
    end
    if AimbotManager.Cache.FOVCircle then
...
local function InitializeAimbot()
    -- Input Handling
    AimbotManager.Cache.InputConnection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if not gameProcessed and input.UserInputType == AimbotManager.Config.Keybind then
            AimbotManager.Cache.IsActive = true
        end
    end)

    AimbotManager.Cache.InputConnection = UserInputService.InputEnded:Connect(function(input)
    ...
    AimbotManager.Cache.Connection = RunService.RenderStepped:Connect(function(deltaTime)
        if not AimbotManager.Config.Enabled or not AimbotManager.Cache.IsActive then
            UpdateFOVCircle()
            return
        end
        ...
        if target then
            ...
            if head then
                ...
                CurrentCamera.CFrame = currentCFrame:Lerp(targetCFrame, 1 - AimbotManager.Config.Smoothness)
            end
        end
