...
-- Sheriff Tab (Updated with Fixed Shot Button and Faster Shot)
...
-- Helper: Shoot Murderer (Optimized)
local function ShootMurderer()
    ...
    if shotType == "Teleport" then
        ...
        task.wait(0.05) -- Minimal wait for teleport
    end
...
-- Helper: Create Shot Button (Fixed Dragging and Immediate Shot)
    ...
    UserInputService.InputChanged:Connect(function(input)
        if input == dragInput and dragging then
            ...
            shotButtonFrame.Position = UDim2.new(
                0, math.clamp(newPos.X.Offset, 0, guiSize.X - buttonSize),
                0, math.clamp(newPos.Y.Offset, 0, guiSize.Y - buttonSize)
            )
        end
    end)
