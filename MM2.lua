...
-- Sheriff Tab (Replacement for existing Sheriff Tab section)
...
local function ShootMurderer()
    ...
    if not gun and shotType == "Default" then
        Notify("Sheriff System", "No gun equipped!", "x-circle")
        return
    end

    if gun and gun.Parent ~= localCharacter then
    ...
    gun = localCharacter:FindFirstChild("Gun") -- Re-check after teleport
    if gun and gun:FindFirstChild("KnifeLocal") and gun.KnifeLocal:FindFirstChild("CreateBeam") then
        ...
        if targetPart then
            pcall(function()
                gun.KnifeLocal.CreateBeam:FireServer(1, targetPart.Position, "AH2")
                Notify("Sheriff System", "Shot fired at murderer!", "check-circle")
            end)
        else
            Notify("Sheriff System", "Failed to find target part!", "x-circle")
        end
    else
        Notify("Sheriff System", "Gun not properly equipped or missing components!", "x-circle")
    end
end
